...
async def refresh_village_data(db: AsyncSession, village_id, lat: float = None, lon: float = None, crops: list | None = None, state: str = None, district: str = None):
    ...
    if lat is None or lon is None:
        try:
            res = await db.execute(text("SELECT latitude, longitude FROM villages WHERE id = :id LIMIT 1"), {'id': str(village_id)})
            row = res.first()
            if row:
                lat, lon = row[0], row[1]
        except Exception:
    ...
    # Single transaction — children must NOT commit individually
    try:
        try:
            saved['weather'] = await ingest_weather(db, village_id, lat, lon, auto_commit=False)
        except Exception:
        ...
        if crops:
            for c in crops:
                try:
                    s = await ingest_market(db, village_id, state or '', district or '', c, auto_commit=False)
