...
def fetch_market(payload: MarketCreate, db: Session = Depends(get_db)):
    try:
        rec = create_market_entry(db, payload.commodity, payload.price, payload.market_name)
        return rec
...
@router.post('/ingest')
async def ingest(village_id: str, state: str, district: str, commodity: str, db: AsyncSession = Depends(get_async_db)):
    try:
