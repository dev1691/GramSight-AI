...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [villagesRes, marketRes, riskRes, statsRes] = results;
    ...
    if (marketRes.status === 'fulfilled') {
      ...
      if (mt && Array.isArray(mt.labels) && mt.labels.length > 0) {
        setMarketTrendData(mt);
      }
    }
    ...
    if (riskRes.status === 'fulfilled') {
      ...
      if (rt && Array.isArray(rt.labels) && rt.labels.length > 0) {
        setRiskTrendData(rt);
      }
    }
    ...
    if (statsRes.status === 'fulfilled') {
      ...
      if (st?.farmer_count != null) {
        setFarmerCount(st.farmer_count);
      }
    }
